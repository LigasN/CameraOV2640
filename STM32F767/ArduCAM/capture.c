...
////Control the CS pin
void CS_HIGH(void) { GPIO_SetBits(CS_PORT, CS_PIN); }

void CS_LOW(void) { GPIO_ResetBits(CS_PORT, CS_PIN); }
...
static unsigned frameBufferLength = 160000u;
...
void StartBMPcapture(void)
{
    ...
    for (int temp = 0; temp < BMPIMAGEOFFSET; temp++)
    {
        while (USART_GetFlagStatus(USART3, USART_FLAG_TXE) == RESET)
            ;
        USART_SendData(USART3, pgm_read_byte(&bmp_header[temp]));
    }
    // CS_LOW();
    HAL_Delay(5000);
    printf("DCMI suspend status: ", HAL_DCMI_Suspend(phdcmi));
    printf("DCMI stop status: ", HAL_DCMI_Stop(phdcmi));
    // set_fifo_burst();
    char VH, VL;
    int i = 0, j = 0;
    for (i = 0; i < 240; i++)
    {
        for (j = 0; j < 320; j++)
        {
            ...
            while (USART_GetFlagStatus(USART3, USART_FLAG_TXE) == RESET)
                ;
            USART_SendData(USART3, VL);
            delay_us(15);
            while (USART_GetFlagStatus(USART3, USART_FLAG_TXE) == RESET)
                ;
            USART_SendData(USART3, VH);
            delay_us(15);
        }
    }
    while (USART_GetFlagStatus(USART3, USART_FLAG_TXE) == RESET)
