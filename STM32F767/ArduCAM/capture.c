...
#define frameBufferLength 160000u
uint8_t frameBuffer[frameBufferLength];
void StartBMPcapture(void)
{
    ...
    HAL_DCMI_Start_DMA(&hdcmi, DCMI_MODE_SNAPSHOT, (uint32_t)frameBuffer,
                       frameBufferLength);
    // uprintf("ACK CMD capture done\r\n");
    // length = read_fifo_length();
    //	uprintf("ACK CMD the length is %d\r\n",length);
    tmp = 0xff;
    while (HAL_UART_Transmit_IT(&huart3, &tmp, 0x01) == HAL_BUSY)
        ;
    tmp = 0xaa;
    while (HAL_UART_Transmit_IT(&huart3, &tmp, 0x01) == HAL_BUSY)
        ;
    while (HAL_UART_Transmit_IT(&huart3, &bmp_header[0], BMPIMAGEOFFSET) ==
           HAL_BUSY)
        ;
    // CS_LOW();
    HAL_Delay(5000);
    uprintf("DCMI suspend status: %d", HAL_DCMI_Suspend(&hdcmi));
    uprintf("DCMI stop status: %d", HAL_DCMI_Stop(&hdcmi));
    // set_fifo_burst();
    unsigned char VH, VL;
    int i = 0, j = 0;
    for (i = 0; i < 240; i++)
    {
        for (j = 0; j < 320; j++)
        {
            VH = frameBuffer[2 * i * j];
            VL = frameBuffer[2 * i * j + 1];
            while (HAL_UART_Transmit_IT(&huart3, &VL, 0x01) == HAL_BUSY)
                ;
            delay_us(15);
            while (HAL_UART_Transmit_IT(&huart3, &VH, 0x01) == HAL_BUSY)
                ;
            delay_us(15);
        }
    }
...
{
    DMA_ClearITPendingBit(DMA1_IT_GL4 | DMA1_IT_TC4 | DMA1_IT_GL5 |
                          DMA1_IT_TC5);
